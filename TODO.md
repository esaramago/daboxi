[X] Adicionar username no header
[X] Adicionar movimento dá erro
[X] Cores das categorias dos movimentos
[X] Balanço não está a mostrar os valores corretos
[X] Exportar estatísticas não está a funcionar
[X] Migrar Transactions para Appwrite
[X] Balanço não está a mostrar os valores corretos
[X] Gastos por categoria não está a mostrar os valores corretos
[X] Ordenar sub-categorias: "Sem categoria" em último lugar
[X] Permitir editar data
[X] Permitir editar valor
[ ] Permitir mudar valor de positivo para negativo ou vice-versa
[ ] Obter apenas movimenntos do user logado
[ ] Integrar API EnableBanking
[ ] Adicionar cache para as tabelas 'types', 'categories' e 'subCategories'
[ ] Offline first: https://appwrite.io/integrations/replication-rxdb